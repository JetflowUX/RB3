      ...
      <div className="flex w-[200%] animate-marquee">
        <div className="flex items-center justify-around w-1/2">
          {items.slice(0, items.length / 2).map((phrase, i) => (
            <Fragment key={i}>
              <span className="text-2xl md:text-4xl font-serif italic text-gold whitespace-nowrap px-6 md:px-8">
                {phrase}
              </span>
        ...
        <div className="flex items-center justify-around w-1/2">
          {items.slice(items.length / 2).map((phrase, i) => (
            <Fragment key={`dup-${i}`}>
              <span className="text-4xl md:text-5xl font-serif italic text-gold whitespace-nowrap px-8">
                {phrase}
              </span>
              <Ornament className="w-6 h-6 text-gold/40 shrink-0" />
            </Fragment>
