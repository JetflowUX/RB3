...
export function Navbar() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "py-4 sm:py-5 glass-panel border-b border-gold/10" : "py-4 sm:py-6 bg-transparent"}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="md:hidden p-1.5 text-white/80 hover:text-gold transition-colors"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            <div className="flex flex-col px-4 sm:px-6 py-6 sm:py-8 gap-6">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
